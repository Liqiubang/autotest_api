...
from config import EXPECTED_CODE


def run_case(results, client, name, api, body):
    """执行单个接口用例并记录结果。

    无论成功、业务失败还是请求异常，都会向 results 追加一条记录，
    保证测试报告覆盖所有接口。返回响应 JSON。
    """
    ...
    try:
        response, url, headers, req_body = client.post(api, body)
...
def pytest_sessionfinish(session, exitstatus):
    ...
    for item in session.items:
        ...
        if cls and hasattr(cls, 'test_results') and cls not in seen_classes:
            seen_classes.add(cls)
            all_results.extend(cls.test_results)
    ...
    env_url = all_results[0]['url'].split('/sms')[0] if all_results else 'N/A'
    html = f'''<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .header {{ background: linear-gradient(135deg, #1a73e8, #4285f4); color: #fff;
        ...
        .url {{ word-break: break-all; font-size: 12px; color: #1a73e8; }}
        .status-badge {{ display: inline-block; padding: 3px 10px; border-radius: 12px;
                         color: #fff; font-size: 12px; font-weight: 600; }}
        .toggle {{ cursor: pointer; color: #1a73e8; font-size: 12px; font-weight: 600;
                   user-select: none; }}
...
<body>
    ...
    <div class="footer">接口自动化测试报告 &copy; {datetime.now().year}</div>
</body>
</html>'''
...
def _json_safe(obj):
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            if isinstance(v, str) and len(v) > 200 and v.startswith('data:image'):
